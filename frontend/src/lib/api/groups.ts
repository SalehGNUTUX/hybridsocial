...
// `reason` is only meaningful for a staff (instance-moderator) deletion of
// someone else's group: the backend opens a takedown + notifies the owner
// (who can then appeal) when a staff actor deletes with a reason. An owner
// deleting their own group passes none and no takedown is created.
